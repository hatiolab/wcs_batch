...
public class MPS3Result implements IExtractor {
	...
	private ArrayList<EquipmentResultVo> extractOrg(String yyyymmdd) {
		...
		sql.add("       TWSBS.DC_CD,");
		sql.add("       TWSBS.JOB_BATCH_SEQ,");
		sql.add("       TWSBS.DC_CD || '_' || TWSBS.JOB_BATCH_SEQ ORD,");
		sql.add("       TWSBS.PLAN_BOX,");
		sql.add("       ACTUAL_BOX BOX,");
		sql.add("       TWSBS.PLAN_PCS,");
		sql.add("       ACTUAL_PCS PCS,");
		sql.add("       TWSBS.PLAN_SKU,");
		sql.add("       ACTUAL_SKU SKU,");
		...
		sql.add("   FROM");
		sql.add("       TB_WORK_STATUS_BY_SITE TWSBS");
		sql.add("       LEFT JOIN (");
		sql.add("           SELECT");
		sql.add("               TB_JOB_BATCH.*,");
		sql.add("               TO_CHAR(TO_DATE(JOB_DATE, 'YYYY-MM-DD'), 'YYYYMMDD') AS BDATE");
		sql.add("           FROM");
		sql.add("               TB_JOB_BATCH");
		sql.add("       ) TJB ON TWSBS.JOB_DATE = TJB.BDATE");
		...
		sql.add("           DC_CD,");
		sql.add("           COM_CD,");
		sql.add("           REGION_CD, COUNT(1) AS CUST");
		sql.add("       FROM");
		sql.add("           (");
		sql.add("               SELECT");
		sql.add("                   DC_CD,");
		...
		sql.add("                           TP.*");
		sql.add("                       FROM");
		sql.add("                           TB_PROCESS TP INNER JOIN RSLT R ON TO_CHAR(TO_DATE(TP.JOB_DATE, 'YYYY-MM-DD'), 'YYYYMMDD') = R.BDATE");
		sql.add("                           AND TO_CHAR(TP.JOB_BATCH_SEQ) = R.JOB_BATCH_SEQ");
		sql.add("                           AND TP.DC_CD = R.DC_CD");
		sql.add("                       WHERE");
		sql.add("                           DOMAIN_ID IN (11, 12)");
		sql.add("                   ) TP_WITH_RSLT");
		sql.add("               WHERE");
		sql.add("                   (CUST_CD, COM_CD, REGION_CD) NOT IN (");
		sql.add("                       SELECT");
		sql.add("                           TB_LOCATION.CUST_CD,");
		sql.add("                           TB_LOCATION.COM_CD,");
		sql.add("                           TB_LOCATION.REGION_CD");
		sql.add("                       FROM");
		sql.add("                           TB_LOCATION");
		sql.add("                       WHERE");
		sql.add("                           1 = 1");
		sql.add("                           AND DOMAIN_ID IN (11, 12)");
		sql.add("                           AND TB_LOCATION.CUST_CD IS NOT NULL");
		sql.add("                   )");
		sql.add("               GROUP BY");
		sql.add("                   TP_WITH_RSLT.COM_CD,");
		sql.add("                   TP_WITH_RSLT.DC_CD,");
		sql.add("                   TP_WITH_RSLT.CUST_CD,");
		sql.add("                   TP_WITH_RSLT.REGION_CD");
		sql.add("           )");
		sql.add("       GROUP BY");
		sql.add("           DC_CD,");
		sql.add("           COM_CD,");
		sql.add("           REGION_CD");
		sql.add("   ) O");
		sql.add("   LEFT JOIN RSLT ON RSLT.DC_CD = O.DC_CD");
		sql.add("   AND RSLT.COM_CD = O.COM_CD");
		sql.add("   AND RSLT.EQUIP_ID = O.REGION_CD");
